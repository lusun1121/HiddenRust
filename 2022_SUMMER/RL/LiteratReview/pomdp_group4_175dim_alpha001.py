...
class FullLikelihood(object):
    ...
    def fl_costs(self, params,theta2,p0,p1,beta = 0.9):#beta=0.9999):
        ...
        for i in range(1000):
            ...
            ecost = self.alpha*(np.log(np.exp((-EV+m)/self.alpha).sum(1) )-(m/self.alpha).T+0.5772)
            ...
            if np.max(np.abs(EV-EV_new))/np.max(np.abs(EV)) <=1e-3:
                break
            
            #print(i,np.max(np.abs(EV-EV_new))/np.max(np.abs(EV)))
    ...
    def fit(self):
        ...
        # self.fitted_full = opt.brute(self.loglike_full, bounds,Ns=5)

        # #self.theta2 = self.fitted_full.x

        self.theta2 = self.fitted_full = np.array([0.94947771 ,0.01198393])
