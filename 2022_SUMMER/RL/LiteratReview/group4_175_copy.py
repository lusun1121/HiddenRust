...
class FullLikelihood(object):
    ...
    def trans(self, theta2,p0,p1):
        ...
        return trans_mat
    #def SigmaLambda(self,x,,theta2,p0,p1):
    
    def belief(self,theta2,p0,p1, path ,data_gene_status = False):
        """
        ...
        """
        if data_gene_status:
            #path = [[zold,action,belief0],[znew,0,0]]
            length = 2
        ...
        for i in range(length-1):
            ...
            if path[i][1] == 1:
                ...
                if gap == 0:
                    ...
                    x.append(x_new)
        return x
    def simulation(self, SampleSize, TimePeriod, reward = [9.243,0.226,1.164],theta2 = [0.949,0.012],\
                   p0 = [0.039,0.333,0.590],p1=[0.181,0.757,0.061]):
        condition = np.zeros([SampleSize,TimePeriod],dtype = int)
        state = np.zeros([SampleSize,TimePeriod],dtype = int )
        x = np.ones([SampleSize,TimePeriod])
        action = np.zeros([SampleSize,TimePeriod],dtype = int)
        rho = 1
        print("generate Q")
        Q = self.fl_costs( reward, theta2, p0, p1 )
        pchoice =self.choice_prob(rho,Q) # z = 0, x= 0, 0.1, 0.2,....,1, z = 1, x = 0,...,1
            ...
            for t in range(TimePeriod-1):
                ...
                if action[nbus,t]==1:
                    ...
                    if condition[nbus,t] == 0:
                        ...
                        path[1,0] = state[nbus,t+1]
                x[nbus,t+1] = self.belief(theta2,p0,p1, path ,data_gene_status = True)[-1]
        ...
        return dat_new,{'state':state,'action':action,'hidden':condition,'belief':x}
    ...
    def fl_costs(self, params, theta2, p0, p1 ,beta=0.9999, threading = 0.45):
        ...
        Output:
            ...
            * EV_new: A (length x 2) array of forward-looking costs associated 
                    with each state z and each belief.
        """
    ...
    def loglike(self, parameters, p0, p1, theta2):
        ...
        # beli_f = self.oneMatrix * np.floor(beli_array)
        # beli_c = self.oneMatrix * np.ceil(beli_array)
        
        # bool_to_int_f = np.array((mat_mij + \
        #                           np.array(beli_f==self.D1,dtype=int)) == 2,
        #                           dtype = int)
        # bool_to_int_c = np.array((mat_mij + \
        #                           np.array(beli_c==self.D1,dtype=int)) == 2,
        #                           dtype = int)
...
datagenerate = FullLikelihood(dat3,'action','state', 
                        dim_x =51,dim_z = 175,time = time, nbus = nbus, 
                        hide_state = True, disp_status = False)
res = datagenerate.simulation(nbus,time)
...
#Initialize
estimation = FullLikelihood(res[0],'action','state', 
                        dim_x =51,dim_z = 175,time = time, nbus = nbus, 
                        hide_state = True, disp_status = False)
...
data_record.append(estimation.fitted2.x)            #theta3 theta2
...
# #     with open(path+'data/seed_new/seed_tol.txt', "wb") as fp:   #Pickling
